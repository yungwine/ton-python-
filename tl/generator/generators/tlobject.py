...
BASE_TYPES = ('string', 'bytes', 'int', 'long', 'int128',
              'int256', 'double', 'Bool', 'true')


def get_name_namespace_constructors(t, arg, type_constructors):
    namespace = ''
    name = arg.type
        ...
        with file.open('w') as f, SourceBuilder(f) as builder:
            ...
            for t in tlobjects:
                if not t.is_function:
                    ...
                    constructors = type_constructors[type_name] or type_constructors[t.result]
...
def _write_class_init(tlobject, kind, type_constructors, builder):
    ...
    args = ['{}: {}{}'.format(
        a.name, a.type_hint(), '=None' if a.flag else '')
...
def _write_from_dict(tlobject, type_constructors: dict, builder):
    builder.writeln('@classmethod')
    builder.writeln('def from_dict(cls, d: dict):')
    builder.writeln(f'assert d.get("_") == "{tlobject.class_name}"')

    for arg in tlobject.args:
        if arg.type in BASE_TYPES:
            builder.writeln('{} = d.get("{}")', '_' + arg.name, arg.name)
        else:
            ...
            if not arg.skip_constructor_id:
                ...
                builder.writeln('from ..types{} import {}', namespace, ', '.join(class_names))

                s = '_' + arg.name + ' = '
                if arg.is_vector:
                    s += f'[] if d.get("{arg.name}") is None else ['
                    for c in class_names:
                        s += f' {c}.from_dict(x) if x["_"] == "{c}" else'
                    s += f' x for x in d.get("{arg.name}")]'
                else:
                    for c in class_names:
                        s += f' {c}.from_dict(d.get("{arg.name}")) if d["_"] == "{c}" else'
                    s += f' d.get("{arg.name}")'
                builder.writeln(s)
            else:
                name = 'Type' + name
                builder.writeln('from ..types{} import {}', namespace, name)
                if not arg.is_vector:
                    builder.writeln(
                        f'{'_' + arg.name} = {name}.from_dict(d.get("{arg.name}")) '
                        f'if isinstance(d.get("{arg.name}"), dict) else d.get("{arg.name}")'
                    )
                else:
                    builder.writeln(
                        f'{'_' + arg.name} = [] if d.get("{arg.name}") is None else [{name}.from_dict(x) '
                        f'if isinstance(x, dict) else x for x in d.get("{arg.name}")]',
                    )
    ...
    if arg.is_vector:
        ...
        if not arg.skip_constructor_id:
            ...
            if sep_index == -1:
                ...
                ns, t = '.' + arg.type[:sep_index], arg.type[sep_index+1:].replace('.', '_')
...
def generate_tlobjects(tlobjects, fn: str, import_depth, output_dir):
